...
import com.example.mykotlinproject.databinding.FragmentMapsBinding
...
class MapsFragment : Fragment() {
    ...
    private fun addMarkerToArray(location: LatLng) {
        val marker: Marker? = setMarker(location, markers.size.toString(), R.drawable.ic_map_pin)
        markers.add(marker)
    }
    ...
    private fun drawLine() {
        ...
        if (last >= 1) {
            val previous: LatLng = markers[last - 1]?.position ?: LatLng(52.52000659999999, 13.404953999999975)
            val current: LatLng = markers[last]?.position ?: LatLng(52.52000659999999, 13.404953999999975)
            map.addPolyline(
                PolylineOptions()
